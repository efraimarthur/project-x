import Link from "next/link";
import React from "react";

export default function Login() {
  return (
    <div className="bg-ter ">
      <div className=" bg-dark text-white shadow p-3 mb-5  rounded bg-opacity-50 p-5 w-50  position-absolute top-50 start-50 translate-middle ">
        <div className=" mb-3 row  ">
          <h1 className="text-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="currentColor" class="bi bi-box-arrow-in-right" viewBox="0 0 16 16">
              <path
                fill-rule="evenodd"
                d="M6 3.5a.5.5 0 0 1 .5-.5h8a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-8a.5.5 0 0 1-.5-.5v-2a.5.5 0 0 0-1 0v2A1.5 1.5 0 0 0 6.5 14h8a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-8A1.5 1.5 0 0 0 5 3.5v2a.5.5 0 0 0 1 0v-2z"
              />

              <path fill-rule="evenodd" d="M11.854 8.354a.5.5 0 0 0 0-.708l-3-3a.5.5 0 1 0-.708.708L10.293 7.5H1.5a.5.5 0 0 0 0 1h8.793l-2.147 2.146a.5.5 0 0 0 .708.708l3-3z" />
            </svg>
            <p>Masuk</p>
          </h1>
        </div>

        <div class="d-flex justify-content-center d-grid gap-2">
          <Link href={"/login/kepsek"} class="btn btn-info w-50 btn-opacity-50 rounded fw-bold" type="button">
            Sebagai Kepala Sekolah
          </Link>
          <Link href={"/login/guru"} class="btn btn-info w-50 btn-opacity-50 rounded fw-bold" type="button">
            Sebagai Guru
          </Link>
          <Link href={"/login/walikelas"} class="btn btn-info w-50 btn-opacity-50 rounded fw-bold" type="button">
            Sebagai Wali Kelas
          </Link>
          <Link href={"/login/ppdb"} class="btn btn-info w-50 btn-opacity-50 rounded fw-bold" type="button">
            Sebagai Panitia PPDB
          </Link>
          <Link href={"/login/siswa"} class="btn btn-info w-50 btn-opacity-50 fw-bold" type="button">
            Sebagai Siswa
          </Link>
        </div>
      </div>
    </div>
  );
}
